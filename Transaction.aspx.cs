...
public partial class Transaction : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            {
                // Filter the SqlDataSource to show only the transactions related to the passed PhoneNumber
                SqlDataSource1.SelectCommand = "SELECT * FROM Sales WHERE PhoneNumber = @PhoneNumber";
                SqlDataSource1.SelectParameters.Clear();
                SqlDataSource1.SelectParameters.Add("PhoneNumber", phoneNumber);

                // Update the statistics for the selected customer
                UpdateStats(phoneNumber);
            }
            else
            {
                // If no PhoneNumber is passed, show statistics for all customers
                UpdateStats(null);
            }
        }
    }
    ...
    private void UpdateStats(string phoneNumber)
    {
        using (SqlConnection conn = new SqlConnection(connectionString))
        {
            ...
            string query = phoneNumber == null
                ? "SELECT SUM(CONVERT(DECIMAL, TotalAmount)), SUM(CONVERT(DECIMAL, BalanceDue)) FROM Sales"
                : "SELECT SUM(CONVERT(DECIMAL, TotalAmount)), SUM(CONVERT(DECIMAL, BalanceDue)) FROM Sales WHERE PhoneNumber = @PhoneNumber";
            ...
            using (SqlCommand cmd = new SqlCommand(query, conn))
            {
                ...
                using (SqlDataReader reader = cmd.ExecuteReader())
                {
                    if (reader.Read())
                    {
                        ...
                        lblTotalSales.Text = "₹" + (reader.IsDBNull(0) ? "0" : Convert.ToDecimal(reader[0]).ToString("N2"));
    ...
    protected void btnAction_Click(object sender, EventArgs e)
    {
        ...
        Response.Redirect("CustomerBillDetails.aspx?BillNumber=" + billNumber);
    }
    ...
    protected void Button2_Click1(object sender, EventArgs e)
    {
        ...
        if (!string.IsNullOrEmpty(searchTerm))
        {
            ...
            SqlDataSource1.SelectCommand = "SELECT * FROM Sales WHERE PhoneNumber = @SearchTerm OR BillNumber = @SearchTerm";
            SqlDataSource1.SelectParameters.Clear();
    ...
    protected void Button4_Click(object sender, EventArgs e)
    {
        ...
        {
            conn.Open();
            using (SqlCommand cmd = new SqlCommand("DELETE FROM Sales WHERE BillNumber = @BillNumber", conn))
            {
    ...
    protected void btnBack_Click(object sender, EventArgs e)
    {
            ...
            // If there is no referrer, navigate to a default page
